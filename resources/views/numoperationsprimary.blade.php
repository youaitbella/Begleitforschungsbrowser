...
@section('titel')
    C-1-2-3: Häufigkeiten von Operationen (4-Steller nach OPS Prozedurengruppe 5) bei Versorgung in Hauptabteilungen - OPS Version {{$year}}, Datenjahr {{$year}}
@endsection
                    ...
                    <table id="tabelle_1" class="table table-bordered table-striped">
                        ...
                        @foreach($data as $entry => $value)
                            <tr>
                                ...
                                <td data-sort="{{$value[10]}}" class="text-right">{{number_format(str_replace(',','.',$value[10]) * 100, 2, ',', '.') . ' %'}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
